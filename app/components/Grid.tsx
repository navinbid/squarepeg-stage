...
export function Grid({
  ...
  gap?: 'default' | 'blog';
  items?: number;
  layout?: 'default' | 'products' | 'auto' | 'blog' | 'collections';
  [key: string]: any;
}) {
  const layouts = {
    ...
    products: `grid-cols-2 ${items >= 3 && 'md:grid-cols-3'} ${
      items >= 4 && 'lg:grid-cols-3 xl:grid-cols-3'
    }`,
    collections: `grid-cols-1 sm:grid-cols-2 ${
      items >= 3 && 'md:grid-cols-3'
    } ${items >= 4 && 'lg:grid-cols-4 xl:grid-cols-6'}`,
    auto: 'auto-cols-auto',
    blog: 'grid-cols-1 md:grid-cols-2',
  };

  const gaps = {
    default: 'grid gap-4 gap-y-6 md:gap-4 lg:gap-6',
    blog: 'grid gap-6',
  };
